...
- [x] **献立カート** — OGP追加 / スクショ欄新設（8言語連動）/ 共通部品へ移行
- [ ] フォーカスジム — OGP追加 / スクショ差し替え（旧UIのため）/ 共通部品へ移行
- [ ] 食べメモ — OGP追加 / スクショ欄新設 / 共通部品へ移行
- [ ] Code Tweet — 既存の日本語og:を英語へ置換 / og:を書き換えるインラインJSを削除 / スクショ欄新設
- [ ] `focus-gym/assets/i18n.js` の削除（フォーカスジム移行後）
- [ ] `kondate-cart/how-to-use.html` の言語切替を共通部品へ（8言語ぶんHTML重複の集約。最後にまとめて）
